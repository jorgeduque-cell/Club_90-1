// ============================================
// CLUB 90 — Admin Teams & Roster Page
// ============================================
...
export default function AdminTeamsPage() {
  ...
  // Form state
  const [teamForm, setTeamForm] = useState<CreateTeamInput>({ name: '', short_name: '', league: 'Torneo Local', color: '#1475e1' });
  ...
  function openAddTeam() {
    setTeamForm({ name: '', short_name: '', league: 'Torneo Local', color: '#1475e1' });
    ...
    <main className="pt-4 pb-24 px-4 max-w-3xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/')} className="text-[#aac7ff] hover:text-white transition-colors">
        ...
        <button
          onClick={openAddTeam}
          className="bg-[#00e601] text-[#013a00] px-4 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider flex items-center gap-1.5 active:scale-95 shadow-[0_4px_12px_rgba(0,230,1,0.25)]"
        >
      ...
      <div className="flex gap-4 flex-col lg:flex-row">
        ...
        <div className="flex-1 space-y-2">
          <p className="text-[9px] font-black text-[#c1c6d5] uppercase tracking-[0.2em] px-1">Equipos del Torneo</p>
          ...
          {loading ? (
            ...
            <div className="bg-[#1a2c39] rounded-xl p-8 text-center">
              <span className="material-symbols-outlined text-4xl text-[#414753] mb-3 block">groups</span>
              <p className="text-[#c1c6d5] text-sm font-bold mb-1">Sin equipos registrados</p>
              <p className="text-[#c1c6d5] text-[10px]">Crea el primer equipo con el botón de arriba</p>
            </div>
          ) : (
            <div className="bg-[#0f212e] rounded-xl overflow-hidden">
              {teams.map((team) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 border-b border-[#414753]/10 last:border-0 hover:bg-[#1a2c39] transition-colors active:bg-[#253744] text-left ${
                    selectedTeam?.id === team.id ? 'bg-[#1a2c39] border-l-4 border-l-[#00e601]' : ''
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-black text-sm"
                      style={{ backgroundColor: team.color || '#1475e1' }}
                    >
                      {team.short_name}
                    </div>
                    <div>
                      <p className="text-white text-sm font-bold">{team.name}</p>
                      <p className="text-[#c1c6d5] text-[10px] font-medium">{team.league}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); openEditTeam(team); }}
                      className="text-[#aac7ff] hover:text-white p-1 transition-colors"
                    >
          ...
          {selectedTeam ? (
            <>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ backgroundColor: selectedTeam.color || '#1475e1' }}
                  >
                    {selectedTeam.short_name}
                  </div>
                  <div>
                    <h2 className="text-white font-black text-base">{selectedTeam.name}</h2>
                    <p className="text-[#c1c6d5] text-[10px]">{selectedTeam.players.length} jugadores</p>
                  </div>
                </div>
                <button
                  onClick={openAddPlayer}
                  className="bg-[#1475e1] text-white px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1 active:scale-95"
                >
              ...
              {selectedTeam.players.length === 0 ? (
                <div className="bg-[#1a2c39] rounded-xl p-8 text-center">
                  <span className="material-symbols-outlined text-3xl text-[#414753] mb-2 block">person_off</span>
                  <p className="text-[#c1c6d5] text-sm">Sin jugadores en la nómina</p>
                </div>
              ) : (
                <div className="bg-[#0f212e] rounded-xl overflow-hidden">
                  {selectedTeam.players.map((player) => (
                    <div
                      key={player.id}
                      className="flex items-center justify-between p-3.5 border-b border-[#414753]/10 last:border-0 hover:bg-[#1a2c39] transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full overflow-hidden bg-[#253744] flex items-center justify-center flex-shrink-0">
                          {player.photo_url ? (
                            <img src={player.photo_url} alt={player.name} className="w-full h-full object-cover" />
                          ) : (
                            <span className="material-symbols-outlined text-[#c1c6d5]">person</span>
                          )}
                        </div>
                        <div>
                          ...
                          <p className="text-[#c1c6d5] text-[10px]">
            ...
            <div className="bg-[#1a2c39] rounded-xl p-10 text-center">
              <span className="material-symbols-outlined text-4xl text-[#414753] mb-3 block">touch_app</span>
              <p className="text-[#c1c6d5] text-sm font-bold">Selecciona un equipo</p>
              <p className="text-[#c1c6d5] text-[10px] mt-1">Haz clic en un equipo para ver su nómina</p>
            </div>
          )}
        </div>
      </div>
        ...
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 z-[95]">
            <div className="bg-[#0f212e] rounded-t-2xl shadow-2xl border-t border-[#253744]">
              <div className="flex justify-center pt-3 pb-1"><div className="w-10 h-1 bg-[#414753] rounded-full" /></div>
              <div className="p-5 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setModal({ type: 'none' })} className="text-[#b1bad3] hover:text-white p-1">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-[#c1c6d5] text-[10px] font-bold uppercase tracking-widest block mb-1">Nombre del Equipo *</label>
                    <input
                      className="w-full bg-[#1a2c39] border border-[#414753]/30 rounded-xl py-3 px-4 text-white font-bold focus:ring-1 focus:ring-[#1475e1]/40 outline-none"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[#c1c6d5] text-[10px] font-bold uppercase tracking-widest block mb-1">Abreviatura *</label>
                      <input
                        className="w-full bg-[#1a2c39] border border-[#414753]/30 rounded-xl py-3 px-4 text-white font-bold focus:ring-1 focus:ring-[#1475e1]/40 outline-none uppercase"
                        placeholder="Ej: LT"
                        maxLength={3}
                    ...
                    <div>
                      <label className="text-[#c1c6d5] text-[10px] font-bold uppercase tracking-widest block mb-1">Color</label>
                      <div className="flex gap-2">
                        <input
                          type="color"
                          className="w-12 h-12 rounded-lg border-0 cursor-pointer bg-transparent"
                          value={teamForm.color || '#1475e1'}
                          onChange={(e) => setTeamForm({ ...teamForm, color: e.target.value })}
                        />
                        <input
                          className="flex-1 bg-[#1a2c39] border border-[#414753]/30 rounded-xl py-3 px-4 text-white font-mono text-sm focus:ring-1 focus:ring-[#1475e1]/40 outline-none"
                  ...
                  <div>
                    <label className="text-[#c1c6d5] text-[10px] font-bold uppercase tracking-widest block mb-1">Liga</label>
                    <select
                      className="w-full bg-[#1a2c39] border border-[#414753]/30 rounded-xl py-3 px-4 text-white font-bold focus:ring-1 focus:ring-[#1475e1]/40 outline-none"
                  ...
                  <div>
                    <label className="text-[#c1c6d5] text-[10px] font-bold uppercase tracking-widest block mb-1">URL del Escudo (opcional)</label>
                    <input
                      className="w-full bg-[#1a2c39] border border-[#414753]/30 rounded-xl py-3 px-4 text-white text-sm focus:ring-1 focus:ring-[#1475e1]/40 outline-none"
                      placeholder="https://..."
                ...
                <button
                  ...
                  disabled={saving}
                  className="w-full bg-[#00e601] text-[#013a00] py-3.5 rounded-xl font-black text-sm uppercase tracking-widest active:scale-[0.98] disabled:opacity-50"
                >
          ...
          <div className="fixed inset-x-0 bottom-0 z-[95]">
            <div className="bg-[#0f212e] rounded-t-2xl shadow-2xl border-t border-[#253744]">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-[#c1c6d5] text-[10px] font-bold uppercase tracking-widest block mb-1">Nombre Completo *</label>
                    <input
                      className="w-full bg-[#1a2c39] border border-[#414753]/30 rounded-xl py-3 px-4 text-white font-bold focus:ring-1 focus:ring-[#1475e1]/40 outline-none"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[#c1c6d5] text-[10px] font-bold uppercase tracking-widest block mb-1">Edad</label>
                      <input
                        className="w-full bg-[#1a2c39] border border-[#414753]/30 rounded-xl py-3 px-4 text-white font-bold focus:ring-1 focus:ring-[#1475e1]/40 outline-none"
                        placeholder="Ej: 25"
                        type="number"
                    ...
                    <div className="flex items-end pb-1">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          className="w-5 h-5 rounded bg-[#1a2c39] border-[#414753] text-[#00e601] focus:ring-[#00e601]/40 cursor-pointer"
                          checked={playerForm.is_captain || false}
                          onChange={(e) => setPlayerForm({ ...playerForm, is_captain: e.target.checked })}
                        />
                        <span className="text-[#c1c6d5] text-xs font-bold">Capitán</span>
                      </label>
                    </div>
                  </div>
                  <div>
                    <label className="text-[#c1c6d5] text-[10px] font-bold uppercase tracking-widest block mb-1">URL de Foto (opcional)</label>
                    <input
                      className="w-full bg-[#1a2c39] border border-[#414753]/30 rounded-xl py-3 px-4 text-white text-sm focus:ring-1 focus:ring-[#1475e1]/40 outline-none"
                      placeholder="https://..."
                ...
                <button
                  ...
                  disabled={saving}
                  className="w-full bg-[#1475e1] text-white py-3.5 rounded-xl font-black text-sm uppercase tracking-widest active:scale-[0.98] disabled:opacity-50"
                >
      ...
      {modal.type === 'confirmDelete' && (
        <>
          ...
          <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 z-[95] max-w-sm mx-auto">
            <div className="bg-[#1a2c39] rounded-2xl p-6 shadow-2xl border border-[#253744] text-center space-y-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 bg-[#253744] text-[#c1c6d5] py-3 rounded-xl font-bold text-sm active:scale-95"
                >
                  Cancelar
                </button>
                <button
