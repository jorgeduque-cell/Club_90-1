...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen pb-20">
      {/* Search Bar */}
      <div className="bg-[#140f0a] px-3 pt-2 pb-0">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#1c1610] border border-[#4a3f2c]/20 rounded-xl py-2.5 pl-10 pr-4 text-white text-sm placeholder:text-[#c2b391]/40 focus:ring-1 focus:ring-[#d72a22]/30 outline-none"
          />
          {searchQuery && (
            <button
      ...
      {/* Sport Filter Tabs */}
      <nav className="bg-[#140f0a] flex overflow-x-auto gap-2 px-3 py-2.5 scrollbar-hide sticky top-16 z-30">
        {FILTERS.map((f) => (
          <button
            key={f.id}
            ...
            className={`rounded-full px-4 py-2 flex-shrink-0 cursor-pointer flex items-center gap-2 transition-all active:scale-95 ${
              activeFilter === f.id
                ? 'bg-[#3a2d1c] text-white shadow-md'
                : 'bg-[#1c1610] text-[#b8a98a] hover:text-white'
            }`}
          >
            <span className="material-symbols-outlined text-[18px]">{f.icon}</span>
            <span className="text-xs font-bold uppercase whitespace-nowrap">{f.label}</span>
            {f.id === 'live' && liveCount > 0 && (
              <span className="bg-[#e5b85c] text-[#2a1c00] text-[8px] font-black rounded-full w-4 h-4 flex items-center justify-center">
                {liveCount}
              </span>
            )}
          </button>
      ...
      <main className="px-3 pt-4 space-y-3">
        ...
        <section className="relative h-40 rounded-xl overflow-hidden mb-4 group cursor-pointer active:scale-[0.99] transition-transform">
          <div className="absolute inset-0 bg-gradient-to-r from-[#0b0805] via-[#0b0805]/60 to-transparent z-10" />
          <img
          ...
          <div className="relative z-20 p-5 flex flex-col justify-center h-full">
            ...
            <button
              ...
              className="bg-[#e5b85c] text-[#2a1c00] w-fit px-5 py-2 rounded-lg text-[11px] font-black uppercase tracking-wider active:scale-95 duration-150 shadow-[0_2px_10px_rgba(0,230,1,0.3)]"
            >
              Pronosticar Ahora
            </button>
          </div>
        ...
        {filteredMatches.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <span className="material-symbols-outlined text-4xl text-[#4a3f2c] mb-3">sports_soccer</span>
        ...
        {supaLoading && live ? (
          ...
          return (
            <div
              key={match.id}
              className={`bg-[#1c1610] rounded-xl p-3.5 space-y-3 transition-all duration-200 ${
            ...
            >
              ...
              <div className="flex justify-between items-center text-[10px] font-bold tracking-tight">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-1 bg-[#140f0a] px-2 py-0.5 rounded-full">
              ...
              >
                <div className="flex flex-col gap-2 flex-1">
                  <div className="flex items-center gap-3">
                    <div className="w-7 h-7 bg-[#2e2418] rounded-full flex items-center justify-center text-[9px] font-black italic text-[#efe6d2]">
                      {match.homeInitials}
                    </div>
                ...
                {match.score && (
                  <div className="flex flex-col items-center gap-0.5 px-4">
                    ...
                    <span className="text-[#4a3f2c] text-[10px]">—</span>
                    <span className="text-white font-black text-lg tabular-nums">{match.score.away}</span>
                  </div>
                )}
                <span className="material-symbols-outlined text-[#4a3f2c] text-sm ml-2 group-hover:text-[#c2b391] transition-colors">chevron_right</span>
              </div>
                    ...
                    <button
                      ...
                      className={`flex justify-between items-center px-3 py-3 rounded-lg transition-all duration-150 group/btn active:scale-95 ${
                        isOddSelected
                          ...
                          : 'bg-[#3a2d1c] hover:bg-[#4a3d28] border border-transparent'
                      }`}
                    >
