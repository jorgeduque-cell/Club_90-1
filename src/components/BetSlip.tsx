// ============================================
// CLUB 90 — PredictionSlip (CONTEXTO §8 Compliant)
// ============================================
// Quick Stakes: [+500], [+1K], [MAX]
// MAX = 2,000 CL (NEVER full balance)
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-[60] transform transition-transform duration-300">
        <div className="bg-[#1a2c39] rounded-t-2xl shadow-[0_-12px_40px_rgba(0,0,0,0.6)] border-t border-[#253744]">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <span className="w-2 h-6 bg-[#00e601] rounded-full" />
            ...
            {/* Selection */}
            <div className="bg-[#0f212e] p-4 rounded-xl border-l-4 border-[#00e601]">
              <div className="flex justify-between items-start mb-1">
                <div>
                  <span className="text-white font-bold text-sm">{selection.predictionLabel}</span>
                  <p className="text-[#b1bad3] text-[11px] mt-0.5">{selection.matchLabel}</p>
                </div>
                <span className="text-[#00e601] font-black text-xl tabular-nums">{selection.multiplier.toFixed(2)}</span>
              </div>
              ...
              {(!live || profile?.accountTier === 'PREMIUM') ? (
                <div className="flex items-center gap-2 mt-4">
                  <div className="flex-1 relative">
                    <input
                      className="w-full bg-[#1a2c39] border border-[#414753]/30 rounded-lg py-3.5 pl-4 pr-12 text-white font-bold focus:ring-1 focus:ring-[#00e601]/40 focus:border-[#00e601]/40 text-lg outline-none tabular-nums"
                      placeholder="Monto"
                      type="text"
                      inputMode="numeric"
                      value={amount}
                    ...
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-[#c1c6d5] uppercase">CL</span>
                  </div>
                  <div className="flex gap-1">
                    <button
                      onClick={() => handleQuickStake('500')}
                      className="bg-[#253744] text-[#b1bad3] px-2 py-3.5 rounded-lg text-[10px] font-bold hover:text-white hover:bg-[#2f4553] transition-colors active:scale-95"
                    >
                      +500
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="bg-[#00e601]/20 text-[#00e601] px-2.5 py-3.5 rounded-lg text-[10px] font-black hover:bg-[#00e601]/30 transition-colors active:scale-95"
                    >
                      MAX
                    </button>
                  </div>
                ...
                <div className="mt-4 bg-[#1a2c39]/50 border border-[#ffd700]/30 rounded-lg p-3 text-center">
                  ...
                  <p className="text-[#b1bad3] text-[9px] mt-1 px-4">Necesitas el Pase Premium para realizar pronósticos.</p>
                </div>
              )}
              ...
              {(!live || profile?.accountTier === 'PREMIUM') && (
                <>
                  ...
                  {numAmount > 0 && isValid && (
                    <p className="text-[#c1c6d5] text-[9px] mt-2 opacity-60">
            ...
            <div className="flex justify-between items-center px-1 mt-2">
              <span className="text-[#b1bad3] text-[11px] font-bold uppercase tracking-widest">
                {(!live || profile?.accountTier === 'PREMIUM') ? 'Beneficio Estimado' : 'Beneficio si fueras Premium'}
              </span>
              <span className="text-[#00e601] font-black text-2xl tracking-tighter tabular-nums drop-shadow-[0_0_8px_rgba(0,230,1,0.2)]">
                {(!live || profile?.accountTier === 'PREMIUM') ? estimatedReturn : (500 * selection.multiplier).toFixed(2)} 🪙
              </span>
            </div>

            <p className="text-[#b1bad3] text-[9px] leading-relaxed opacity-60">
            ...
            {live && profile?.accountTier === 'GUEST' ? (
              ...
              <button
                ...
                className={`w-full font-black text-base py-4 rounded-xl uppercase tracking-widest transition-all duration-200 shadow-lg ${
                  !isValid
                    ? 'bg-[#253744] text-[#c1c6d5]/50 cursor-not-allowed shadow-none'
                    : confirming
                    ? 'bg-[#ff9800] text-[#0f212e] active:scale-[0.98] shadow-[0_4px_16px_rgba(255,152,0,0.3)] animate-pulse'
                    : 'bg-[#00e601] text-[#0f212e] active:scale-[0.98] shadow-[0_4px_16px_rgba(0,230,1,0.3)]'
                }`}
              >
